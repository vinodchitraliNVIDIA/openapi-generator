...
public abstract class AbstractGoOpenApiCodegen extends DefaultCodegen implements CodegenConfig {

    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractGoOpenApiCodegen.class);

    protected boolean withGoCodegenComment = false;
    protected boolean withXml = false;


    public AbstractGoOpenApiCodegen() {
        super();
    ...
    public String toModelFilename(String name) {
        ...
        if (name.endsWith("_test") || name.endsWith("_re_arm")) {
        ...
        for (CodegenOperation operation : operations) {
            for (CodegenParameter param : operation.allParams) {
				if (!modelPackage().isEmpty()) {
					...
					} else if (param.isContainer && param.items.isModel) {
						param.dataType = param.dataType.replaceAll(param.items.dataType,  modelPackage() + "." + param.items.dataType);
					}
                }
            }
	        if (operation.responses != null && !operation.responses.isEmpty()) {
				if (!modelPackage().isEmpty()) {
            		for (CodegenResponse param : operation.responses) {
						if (param.isModel) {
							param.dataType = modelPackage() + "." + param.dataType;
						} else if ((param.containerType == "array" || param.containerType == "map") &&
									(param.baseType != "map" && !languageSpecificPrimitives.contains(param.baseType))) {
								param.dataType = param.dataType.replaceAll(param.baseType,  modelPackage() + "." + param.baseType);
						}
					}
				}
			}
